// src/utils/withTransform.ts

// Defaults for editor-embedded media
export const IMG_WIDTH: number = 900; // max delivery width
export const IMG_QUALITY: number = 80; // 1..100

interface WithTransformOptions {
  width?: number;
  height?: number;
  quality?: number;
  crop?: string;
...
export function withTransform(
  ...
  try {
    ...
    let uploadIdx = parts.findIndex((p: string, i: number) => {
    ...
    if (uploadIdx === -1) uploadIdx = parts.findIndex((p: string) => p === "upload");
    ...
    if (Array.isArray(flags) && flags.length) {
      tokens.push(...flags.map((f: string) => `fl_${f}`));
    }
