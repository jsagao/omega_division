// src/component/PostListItem.tsx
...
function thumbSrcFor(post: PostListItemPost | null): string {
 ...
 */
async function tryCloudinaryDeleteByToken(deleteToken: string): Promise<boolean> {
 ...
 */
async function tryServerDestroy(url: string): Promise<boolean> {
  if (!url) return false;
  try {
...
export default function PostListItem({ post, onDeleted }: PostListItemProps): React.ReactElement | null {
  const { user } = useUser();
  const isAdmin = user?.publicMetadata?.role === "admin";

  const [deleting, setDeleting] = useState<boolean>(false);
  const [gone, setGone] = useState<boolean>(false); // local collapse if parent doesn't remove fast
