// src/routes/PrimarySinglePost.tsx
import React, { useEffect, useMemo, useRef, useState } from "react";
import { Link, useParams, useNavigate } from "react-router-dom";
import { useUser } from "@clerk/clerk-react";
import Image from "../component/Image";
import PostMenuActions from "../component/PostMenuActions";
...
type PostStatus = "loading" | "ok" | "notfound" | "error";
type SeriesStatus = "idle" | "loading" | "ok" | "empty" | "error";
...
function SeriesNav({ series, currentId }: SeriesNavProps): React.ReactElement | null {
  if (!series) return null;

  // Normalize items: backend may return { items: [...] } or { all: [...] }
  const rawItems: SeriesItem[] = Array.isArray(series.items)
    ? series.items
    : Array.isArray(series.all)
      ? series.all
      : [];

  const items: NormalizedSeriesItem[] = rawItems.map((it) => ({
    id: Number(it.id),
  ...
  // Normalize prev/next
  const prev: NormalizedPrevNext | null = series.prev
    ? {
  ...
  const next: NormalizedPrevNext | null = series.next
    ? {
...
export default function PrimarySinglePost(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res: Response = await fetch(`${API}/posts/${id}/series`, { signal: ctrl.signal });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data: SeriesData = await res.json(); // backend returns { series_key, all: [...], prev, next }
  ...
  async function handleDelete(): Promise<void> {
    try {
      ...
      const res: Response = await fetch(`${API}/posts/${id}`, { method: "DELETE" });
