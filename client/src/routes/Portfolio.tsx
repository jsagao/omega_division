...
export default function Portfolio(): React.ReactElement {
  ...
  const [items, setItems] = useState<PortfolioApiItem[]>([]);
  const [status, setStatus] = useState<LoadStatus>("loading");
  const [workingId, setWorkingId] = useState<string | number | null>(null); // track which post is being updated
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setStatus("loading");
        const res: Response = await fetch(`${API}/portfolio?limit=20`);
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const data: PortfolioApiItem[] | unknown = await res.json();
        if (!alive) return;
  ...
  const mapped = useMemo<MappedPost[]>(() => {
    return (items || []).map((p) => {
  ...
  async function handleRemoveFromPortfolio(postId: string | number, postTitle: string): Promise<void> {
    if (!isAdmin) return;
    ...
    try {
      setWorkingId(postId);
      const res: Response = await fetch(`${API}/posts/${postId}`, {
        method: "PATCH",
